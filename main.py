...
for filename in os.listdir('input'):
    if filename.endswith('.csv'):
        ...
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Processing file: {filename}...")
        ...
        with open(input_filepath, 'r', encoding='utf-8') as infile, open(output_filepath, 'w', encoding='utf-8', newline='') as outfile:
            ...
            for line_number, row in enumerate(reader, start=1):
                ...
                if num_commas == correct_num_commas:
                    ...
                    print("")
                    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Skipped invalid row in {filename} at line {line_number + 1}: {truncated_row}...")
                    skipped_rows += 1
        ...
        print("")
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Completed processing file: {filename}")
        print("")
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Processed in {elapsed_time:.2f} seconds")
        print("")
        if skipped_rows > 0:
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {skipped_rows} invalid rows skipped")
            print("")
        print("--------------------------------------------------")
        print("")
